  ...
  try {
    if (method === 'initialize') {
      writeResult(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'node-search', version: '5.1.0' }
      });
      return;
    }
...
const TOOL_DEFINITIONS = [
  {
    name: 'search_files',
    description: 'Busca padrões de texto/regex em arquivos com auto-detecção de encoding. UTF-8, UTF-16 e Windows-1252 são suportados.',
    inputSchema: {
      type: 'object',
      properties: {
        pattern: { type: 'string', description: 'Padrão de texto (regex ou literal)' },
  ...
  },
  {
    ...
    description: 'Busca e substitui texto/regex em um ou mais arquivos. Por padrão roda em modo dryRun (preview). Arquivos modificados são regravados em UTF-8 com backup .bak.[timestamp] opcional.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        backup: { type: 'boolean', description: 'Criar .bak.[timestamp] antes de sobrescrever (default: true). Aborta operação se backup falhar.' },
  ...
  {
    name: 'read_lines',
    description: 'Lê um intervalo específico de linhas de um arquivo (streaming — não carrega o arquivo inteiro). Detecta encoding automaticamente.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'fix_encoding',
    description: 'Detecta e converte arquivos não-UTF-8 (ex: .resx salvos em Windows-1252) para UTF-8. Por padrão dryRun.',
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Arquivo único ou diretório raiz' },
  ...
  {
    name: 'fix_mojibake',
    description: 'Detecta e corrige mojibake por DUPLA codificação (UTF-8 decodificado como Latin-1 e regravado, ex: "São Paulo" → "SÃ£o Paulo"). Por padrão dryRun.',
    inputSchema: {
      type: 'object',
      properties: {
        path: { type: 'string', description: 'Arquivo único ou diretório raiz' },
        filePattern: { type: 'string', description: 'Extensões separadas por vírgula (ex: .resx,.frx). Default: todas' },
        dryRun: { type: 'boolean', description: 'Apenas relatar, sem converter (default: true)' },
        maxFiles: { type: 'number', description: 'Máximo de arquivos a verificar/converter (default: 200, máx 1000)' },
...
async function walk(dir, fileExts, onFileFound) {
  let dirents;
  try {
  ...
  for (const dirent of dirents) {
    ...
    if (dirent.isDirectory()) {
      if (!IGNORE_DIRS.has(dirent.name)) {
        await walk(fullPath, fileExts, onFileFound);
      }
    } else if (dirent.isFile()) {
      const matchesExt = !fileExts || fileExts.some(ext => dirent.name.endsWith(ext));
      if (matchesExt) onFileFound(fullPath);
    }
  }
}

async function collectFiles(targetPath, fileExts) {
  const list = [];
  let stat;
  try {
  ...
  if (stat.isDirectory()) {
    await walk(targetPath, fileExts, (f) => list.push(f));
  } else {
...
async function executeSearch(id, args) {
  ...
  try {
    ...
    return writeToolError(id, `Regex inválida fornecida: ${err.message}`);
  }
  ...
  async function searchInFile(filePath) {
    ...
    for (const rawLine of lines) {
      ...
      const match = regex.test(rawLine);
  ...
  try {
    const fileList = await collectFiles(searchPath, fileExts);
...
async function executeFixEncoding(id, args) {
  ...
  const fileList = await collectFiles(targetPath, fileExts);
  if (fileList.length === 0) {
